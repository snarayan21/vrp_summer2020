...
def graph_solution_curved(data, manager, routing, solution, line):
    ...
    max_route_distance = 0
    num_nodes = data['num_nodes']
    img = mpimg.imread("/Users/saaketh/Desktop/vrp_summer2020/original_road.png")
    plt.imshow(img, extent=[0, 1000, 0, 1000])
    plt.plot([data['x'][k] for k in list(range(1, num_nodes))],[data['y'][k] for k in list(range(1, num_nodes))],'ko',markersize=4)
    plt.xlim(0,data['bound_length'])
    plt.ylim(0, data['bound_length'])
    plt.savefig("curved_road_waypoints_only.png", dpi=dpi)
    plt.show()
    plt.clf()
    plt.close()
    dpi = 192
    ...
    #cmap = plt.get_cmap('gist_rainbow')
    colors = ['k' for i in np.linspace(0,1,data['num_vehicles'])]
    legendlines = []
    for vehicle_id in range(data['num_vehicles']):
        index = routing.Start(vehicle_id)
        route_distance = 0
    ...
    plt.xlim(0,data['bound_length'])
    plt.ylim(0, data['bound_length'])
    plt.savefig("curved_road_with_routes.png", dpi=dpi)
    plt.show()
    plt.clf()
    plt.close()
...
def extract_roadnode_data(data, manager, routing, solution):
    ...
    for vehicle_id in range(data['num_vehicles']):
        ...
        while not routing.IsEnd(index):
            ...
            if(prevnodenum != 0 and nodenum != 0):
                part_route_distance += routing.GetArcCostForVehicle(previous_index, index, vehicle_id)
            
            if (prevnodenum == 0 and nodenum != 0):
                candidates.append(data['roadnodecoords'][nodenum])
                newrouteptsx.append(data['x'][nodenum])
                newrouteptsy.append(data['y'][nodenum])
            elif (prevnodenum != 0 and nodenum == 0):
                candidates.append(data['roadnodecoords'][prevnodenum])
                newrouteptsx.append(data['x'][prevnodenum])
                newrouteptsy.append(data['y'][prevnodenum])
            
        if(route_distance != 0):
            routedistances.append(route_distance)
            partroutedistances.append(part_route_distance)
            usednodecoords.append(candidates[0])
            usednodecoords.append(candidates[1])
            routepointcoords[0].append(newrouteptsx[0])
            routepointcoords[0].append(newrouteptsx[1])
            routepointcoords[1].append(newrouteptsy[0])
            routepointcoords[1].append(newrouteptsy[1])
     
    #numroadnodes = len(usednodecoords)
    
    for i in range(len(routedistances)):
        print("Route", i+1, " distance", routedistances[i], "partroute distance", partroutedistances[i], "endpoints:", usednodecoords[2*i], usednodecoords[2*i + 1], " routepoints:", routepointcoords[0][0], routepointcoords[1][0], " and ", routepointcoords[0][1], routepointcoords[1][1])
    ...
    return routedistances, usednodecoords, routepointcoords, partroutedistances
...
def extract_roadnode_data_curved(data, manager, routing, solution):
    ...
    for vehicle_id in range(data['num_vehicles']):
        ...
        while not routing.IsEnd(index):
            ...
            route_distance += routing.GetArcCostForVehicle(previous_index, index, vehicle_id)
            if (prevnodenum == 0 and nodenum != 0):
                candidatesx.append(data['roadnodecoordsx'][nodenum])
                candidatesy.append(data['roadnodecoordsy'][nodenum])
            elif (prevnodenum != 0 and nodenum == 0):
                candidatesx.append(data['roadnodecoordsx'][prevnodenum])
                candidatesy.append(data['roadnodecoordsy'][prevnodenum])
            
        if(route_distance != 0):
            routedistances.append(route_distance)
            usednodecoords[0].append(candidatesx[0])
            usednodecoords[0].append(candidatesx[1])
            usednodecoords[1].append(candidatesy[0])
            usednodecoords[1].append(candidatesy[1])
...
def solve_singledepotroad(numnodes, numvehicles, dronerange):
    ...
    if solution:
        ...
        routedistances, usednodecoords, routepointcoords, partroutedistances = extract_roadnode_data(data, manager, routing, solution)
        vrpdata = {}
        truckdata = {}
        vrpdata['data'] = data
        ...
        truckdata['routepointcoords'] = routepointcoords
        truckdata['partroutedistances'] = partroutedistances
